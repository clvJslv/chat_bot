...
class DatabaseConnection:
    ...
    def get_perguntas(self, filtro_modulo=None):
        ...
        if filtro_modulo:
            cursor.execute("SELECT PK_CO_PERGUNTA, CO_PERGUNTA,  DE_PERGUNTA FROM TB_007_PERGUNTAS WHERE PK_CO_PERGUNTA = ?", filtro_modulo)
        else:
    ...
    def insert_pergunta(self, pergunta, modulo):
        cursor = self.conn.cursor()
        cursor.execute("INSERT INTO TB_007_PERGUNTAS (CO_PERGUNTA, DE_PERGUNTA) VALUES (?, ?)", pergunta, modulo)
        self.conn.commit()
        cursor.close()

    def update_pergunta(self, id, pergunta, modulo):
        cursor = self.conn.cursor()
        cursor.execute("UPDATE TB_007_PERGUNTAS SET CO_PERGUNTA = ?, DE_PERGUNTA = ? WHERE PK_CO_PERGUNTA = ?", pergunta, modulo, id)
        self.conn.commit()
        cursor.close()
